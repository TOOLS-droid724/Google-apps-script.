...
const EMAIL_STYLES = `
  ...
  .container { max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 0; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); border: 1px solid #e5e7eb; }
  .header { text-align: center; padding: 32px 20px; background-color: #ffffff; border-bottom: 1px solid #e5e7eb; }
  .header img { border-radius: 0; max-width: 80px; }
  ...
  .btn { display: inline-block; padding: 12px 24px; background-color: #0f172a; color: #ffffff; text-decoration: none; border-radius: 0; font-weight: 500; margin-top: 16px; }
  .highlight-box { background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 0; margin: 20px 0; }
  ...
  return `
    <html>
      ...
      <body>
        <div class="container">
          <div class="header">
            <img src="${LOGO_URL}" alt="Aarambh Naturals Logo">
          </div>
          <div class="content">
            ${contentHtml}
          </div>
...
function sendBulkPromoEmail(emailsBCC, title, message, bannerUrl) {
  const bannerHtml = bannerUrl ? `<img src="${bannerUrl}" style="width:100%; border-radius: 0; margin-bottom:24px;">` : "";
  const content = `
    ${bannerHtml}
    <h2>${title}</h2>
...
function sendOTPEmail(email, otp, type) {
  const content = `
    ...
    <div style="text-align: center; margin: 32px 0;">
      <span style="font-size: 32px; font-weight: 700; letter-spacing: 6px; color: #0f172a; background: #f1f5f9; padding: 16px 24px; border-radius: 0;">${otp}</span>
    </div>
...
function sendRegistrationPDF(email, name, phone, userId) {
  ...
  MailApp.sendEmail({ 
    ...
    body: "Welcome to Aarambh Naturals. Please find your official account registration details securely attached to this email.", 
    attachments: [blob] 
  });
}
