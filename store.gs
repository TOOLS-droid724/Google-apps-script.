...
function processOrder(orderData) {
  ...
  if(orderData.razorpayId && orderData.razorpayId !== "COD") {
    try {
      const auth = Utilities.base64Encode("rzp_live_TIWEbq6LlKUdZf:SFRSwjiLdIxkgO02f6rOF2QP");
      const rzpOptions = {
        "method": "get",
        "headers": {
      ...
      if(resCode !== 200 || (rzpData.status !== "captured" && rzpData.status !== "authorized")) {
        return {status: "error", message: "Payment verification failed or payment not captured."};
      }
    } catch(e) {
